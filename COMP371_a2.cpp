...
int main(int argc, char*argv[])
{
	...
	vec3 cameraPosition(0.0f, 10.0f, 25.0f);
	...
	bool lFootFwd = true;
	float lFootOffset = 0.0f;

	bool forToggle = false;
	bool backToggle = false;
	int lastKeyWState = GLFW_RELEASE;
	int lastKeySState = GLFW_RELEASE;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (texEnabled) {
			// Draws Cube, starting at 0.0f, 0.0f, 0.0f, so that it's centered at origin
			worldMatrix = scale(mat4(1.0f), vec3(100.0f, 0.0f, 100.0f));
		...
		glUniform4f(vertexColorLocation, 1.0f, 1.0f, 1.0f, 1.0f);			// Sets fragment color for our vertex array to white

		// Olaf feet
		for (int i = -1; i < 2; i+=2) {
			// Translate feet to olafRotate - 90.0f due to feet positions being perpendicular to facing direction
			if (i == -1) {
				worldMatrix = translate(mat4(1.0f), vec3(olafTransX, 0.0f, olafTransZ)) * 
					translate(mat4(1.0f), vec3(sinf(radians(olafRotateY - 90.0f)) * (i * 0.75f) * olafScale, 0.375f * olafScale, cosf(radians(olafRotateY - 90.0f)) * (i * 0.75f) * olafScale)) * 
					rotate(mat4(1.0f), radians(olafRotateY), vec3(0.0f, 1.0f, 0.0f)) * 
					scale(mat4(1.0f), vec3(1.0f * olafScale, 0.75f * olafScale, 0.75f * olafScale));
			}
			else {
			worldMatrix = translate(mat4(1.0f), vec3(olafTransX, 0.0f, olafTransZ)) * 
				translate(mat4(1.0f), vec3(sinf(radians(olafRotateY - 90.0f)) * (i * 0.75f) * olafScale, 0.375f * olafScale, cosf(radians(olafRotateY - 90.0f)) * (i * 0.75f) * olafScale)) * 
		...
		if (hatEnabled) {
			// Olaf hat brim
			worldMatrix = translate(mat4(1.0f), vec3(olafTransX, 0.0f, olafTransZ)) * translate(mat4(1.0f), vec3(0.0f, 7.9f * olafScale, 0.0f)) *
				rotate(mat4(1.0f), radians(olafRotateY), vec3(0.0f, 1.0f, 0.0f)) * scale(mat4(1.0f), vec3(3 * olafScale, 0.5 * olafScale, 3 * olafScale));
			if (texEnabled) {
				setWorldMatrix(texturedShaderProgram, worldMatrix);
			}
			else {
			...
			glUniform1i(textureLocation, 6);                // Set our Texture sampler to use Texture Unit 6 (metal)
			glDrawArrays(renderMode, 0, 36);

			// Olaf hat top
			worldMatrix = translate(mat4(1.0f), vec3(olafTransX, 0.0f, olafTransZ)) * translate(mat4(1.0f), vec3(0.0f, 8.9f * olafScale, 0.0f)) *
				rotate(mat4(1.0f), radians(olafRotateY), vec3(0.0f, 1.0f, 0.0f)) * scale(mat4(1.0f), vec3(1.5 * olafScale, 2.5 * olafScale, 1.5 * olafScale));
			if (texEnabled) {
				setWorldMatrix(texturedShaderProgram, worldMatrix);
			}
			else {
		...
		if ((glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) && (glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS || glfwGetKey(window, GLFW_KEY_RIGHT_SHIFT)) == GLFW_PRESS) {
			...
			if (lFootFwd) { lFootOffset += 0.02f; }
			else { lFootOffset -= 0.02f; }
			if (abs(lFootOffset) >= 2.0f) { lFootFwd = !lFootFwd; }
		}
		...
		if (lastKeyWState == GLFW_RELEASE && (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) && !(glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS || glfwGetKey(window, GLFW_KEY_RIGHT_SHIFT)) == GLFW_PRESS) {
			backToggle = !backToggle;
		}
		lastKeyWState = glfwGetKey(window, GLFW_KEY_W);
		if (backToggle) {
			olafTransZ += 0.1 * cosf(radians(olafRotateY));
			olafTransX += 0.1 * sinf(radians(olafRotateY));
		}
		if (lastKeySState == GLFW_RELEASE && (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) && !(glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS || glfwGetKey(window, GLFW_KEY_RIGHT_SHIFT)) == GLFW_PRESS) {
			forToggle = !forToggle;
		}
		lastKeySState = glfwGetKey(window, GLFW_KEY_S);
		if (forToggle) {
			olafTransZ -= 0.1 * cosf(radians(olafRotateY));
			olafTransX -= 0.1 * sinf(radians(olafRotateY));
		}
		...
		//Toggles Hat on/off
		if (lastKeyHState == GLFW_RELEASE && glfwGetKey(window, GLFW_KEY_H) == GLFW_PRESS) {
			hatEnabled = !hatEnabled;
		}
		lastKeyHState = glfwGetKey(window, GLFW_KEY_H);

		// Custom-Keybinds to Rotate Olaf at 1/4 Speed
...
// Vertex Buffer Object for the Light VAO
